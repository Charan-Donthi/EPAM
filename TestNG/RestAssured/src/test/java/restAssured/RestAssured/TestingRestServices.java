...
public class TestingRestServices {
	  ...
	  @Test(dataProvider="dataProviderForVerifyingHttpResponse",dependsOnMethods="verifyResponseStatusCodeForPost")
	  public void verifyHttpResponseBodyForPost(String URL,String key) {
		  Response response=httpRequest.get(URL);
		  assertEquals(response.jsonPath().getList(key).size(),11);
	  }
		  
	  
	  @DataProvider
	  public Object[][] dataProviderForVerifyingHttpResponseBodyForPost() {
		  return new Object[][] {
			  {"https://jsonplaceholder.typicode.com/users","id"},
		  };
	  }
